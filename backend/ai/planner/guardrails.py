...
def verify_graph(db: Session, session_id: str, user_prompt: str = "") -> str | None:
    ...
    # Reject unrequested delivery nodes — now covers all four channels, not just email.
    DELIVERY_KEYWORDS = {
        ("gmail", "resend"): ["email", "e-mail", "gmail", "mail", "send an email", "send email", "notify me by email"],
        ("slack",): ["slack"],
