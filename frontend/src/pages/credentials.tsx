...
import { ArrowLeft, KeyRound, Loader2, Plus, Trash2, WalletCards } from "lucide-react";
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/dashboard")} className="rounded p-2 text-slate-400 hover:bg-slate-800 hover:text-white">
              <ArrowLeft className="h-4 w-4" />
            </button>
            <div>
              <h1 className="text-2xl font-bold text-white">Credentials</h1>
              <p className="mt-1 text-sm text-slate-400">Store encrypted API keys for integration nodes.</p>
            </div>
          </div>
          <Link to="/dashboard" className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-300 hover:bg-slate-700">
            Workflows
          </Link>
        </div>

        <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
          <form onSubmit={createCredential} className="rounded-xl border border-slate-800 bg-slate-900/60 p-5">
            ...
            <textarea
              ...
              spellCheck={false}
              className="h-40 w-full resize-none rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 font-mono text-xs text-slate-100 outline-none focus:border-blue-500"
            />
            ...
            <button
              type="submit"
              disabled={saving}
              className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-500 disabled:opacity-60"
            >
                ...
                {credentials.map((credential) => (
                  <div key={credential.id} className="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-950/50 p-4">
                    ...
                    <button
                      ...
                      className="rounded p-2 text-slate-500 hover:bg-red-500/10 hover:text-red-300"
